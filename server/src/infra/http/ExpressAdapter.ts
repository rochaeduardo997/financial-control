...
class ExpressAdapter implements IHttp {
  ...
  private async middleware(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      return res
        .status(401)
        .json({ status: false, result: err.message || "invalid token" });
    }
  }
    ...
    this.route[method](url, async (req: any, res: any) => {
      try {
        ...
        if (/failed on get/.test(err?.message))
          return res.status(404).json({ status: false, result: err?.message });
        else
          return res.status(400).json({ status: false, result: err?.message });
      }
    });
  }
