...
export default class CategoryRepository implements ICategoryRepository {
  ...
  async getBy(id: string, userId: string): Promise<Category> {
    try {
      const result = await this.CATEGORY_MODEL.findOne({
        raw: true,
        where: { id },
      });
      ...
      if (!isFromRespectiveUser)
        throw new Error("cannot access category from another user");
      return this.instanceCategoryFrom(result);
    } catch (err: any) {
      console.error(err);
      throw new Error(
  ...
  async getAllBy(userId: string): Promise<Category[]> {
    try {
      ...
      console.error(err);
      throw new Error(
        err?.errors?.[0]?.message || err.message || "failed on get categories",
      );
    }
  }
  ...
  async updateBy(id: string, input: Category): Promise<Category> {
    try {
      ...
      console.error(err);
      throw new Error(
        err?.errors?.[0]?.message || err.message || "failed on update category",
      );
    }
  }
  ...
  async deleteBy(id: string, userId: string): Promise<boolean> {
    try {
      ...
      if (!verifyIfCategoryExists) throw new Error();
      const result = await this.CATEGORY_MODEL.destroy({
      ...
      console.error(err);
      throw new Error(
        err?.errors?.[0]?.message || "failed on delete category by id",
      );
    }
  }
