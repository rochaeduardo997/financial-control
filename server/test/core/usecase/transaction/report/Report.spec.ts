...
import ReportHandler from "../../../../../src/core/usecase/transaction/report/Report";
...
beforeEach(async () => {
  ...
  reportHandler = new ReportHandler(reportRepository);
...
describe("success", () => {
  test("find all transactions filtering only by date(start and end)", async () => {
    ...
    expect(result[0].categoriesId).toEqual([
