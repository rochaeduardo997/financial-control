...
describe("success", () => {
  ...
  test("find by id", async () => {
    ...
    const result = await transactionRepository.getBy(
      transaction.id,
      users[0].id,
    );
  ...
  test("update transaction without change categories", async () => {
    transactions[0].associateCategory(categories[0]);
    await transactionRepository.create(transactions[0]);
    const result = await transactionRepository.updateBy(
      transactions[0].id,
      transactions[1],
    );
    expect(result.id).toBe(transactions[0].id);
  ...
  test("delete category by id", async () => {
    await transactionRepository.create(transactions[0]);
    await transactionRepository.create(transactions[1]);
    const deleteResult = await transactionRepository.deleteBy(
      transactions[0].id,
...
describe("fail", () => {
  test("create that already exists", async () => {
    await transactionRepository.create(transactions[0]);
    await expect(() =>
      transactionRepository.create(transactions[0]),
    ).rejects.toThrow("id must be unique");
  });

  test("find by invalid id", async () => {
    await expect(() =>
      transactionRepository.getBy("invalid_id", users[0].id),
  ...
  test("find by from another user", async () => {
    ...
    await transactionRepository.create(transactions[1]);
    await expect(() =>
      transactionRepository.getBy(transactions[1].id, users[0].id),
    ).rejects.toThrow("cannot access transaction from another user");
  });

  test("update transaction with invalid id", async () => {
    await expect(() =>
      transactionRepository.updateBy("invalid_id", transactions[0]),
    ).rejects.toThrow("failed on get transaction by id");
  });
  ...
  test("update transaction from another user", async () => {
    ...
    await transactionRepository.create(transactions[1]);
    await expect(() =>
      transactionRepository.updateBy(transactions[1].id, transactions[0]),
    ).rejects.toThrow("cannot access transaction from another user");
  });
  ...
  test("delete transaction by invalid id", async () => {
    ...
    await transactionRepository.create(transactions[1]);
    await expect(() =>
      transactionRepository.deleteBy("invalid_id", "invalid_user_id"),
