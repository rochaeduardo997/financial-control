...
describe("success", () => {
  ...
  test("update category", async () => {
    await categoryRepository.create(categories[0]);
    const result = await categoryRepository.updateBy(
      categories[0].id,
      categories[1],
    );
    expect(result.id).toBe(categories[0].id);
  ...
  test("delete category by id", async () => {
    await categoryRepository.create(categories[0]);
    await categoryRepository.create(categories[1]);
    const deleteResult = await categoryRepository.deleteBy(
      categories[0].id,
      categories[0].userId!,
    );
...
describe("fail", () => {
  test("create that already exists", async () => {
    await categoryRepository.create(categories[0]);
    await expect(() =>
      categoryRepository.create(categories[0]),
    ).rejects.toThrow("id must be unique");
  });

  test("find by invalid id", async () => {
    await expect(() =>
      categoryRepository.getBy("invalid_id", users[0].id),
  ...
  test("find by from another user", async () => {
    ...
    await categoryRepository.create(categories[1]);
    await expect(() =>
      categoryRepository.getBy(categories[1].id, users[0].id),
    ).rejects.toThrow("cannot access category from another user");
  });

  test("update category with invalid id", async () => {
    await expect(() =>
      categoryRepository.updateBy("invalid_id", categories[0]),
    ).rejects.toThrow("failed on get category by id");
  });
  ...
  test("update category from another user", async () => {
    ...
    await categoryRepository.create(categories[1]);
    await expect(() =>
      categoryRepository.updateBy(categories[1].id, categories[0]),
    ).rejects.toThrow("cannot access category from another user");
  });
  ...
  test("delete category by invalid id", async () => {
    ...
    await categoryRepository.create(categories[1]);
    await expect(() =>
      categoryRepository.deleteBy("invalid_id", "invalid_user_id"),
